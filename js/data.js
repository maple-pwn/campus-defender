...
const LEVELS = {
  ...
  'ch1-1': {
    ...
    index: 0,
    title: '初次接触',
    story: '导师说最近教学楼的服务器有异常流量，让你去看看。打开终端，我们先从最基础的开始。',
    isBoss: false,
    objectives: [
      { id: 'help', description: '输入 help 查看可用命令' },
      { id: 'scan', description: '使用 scan teach.cassel.edu 扫描目标' },
      { id: 'connect_80', description: '连接 80 端口查看 Web 服务: connect teach.cassel.edu 80' },
    ],
    target: null,
    expectedCommands: ['help', 'scan', 'connect'],
    mentorMessages: [
      { trigger: 'start', text: '嘿，新来的！我是古德里安教授，今天教你点不一样的。网络安全的第一步叫"侦察"——搜集目标信息。终端里输入 help 看看你手头有哪些工具。' },
    ...
    scanResults: {
      'teach.cassel.edu': [
        ...
        { port: 80, service: 'HTTP', banner: 'nginx 1.18', info: 'Web 服务器，托管学院教务系统' },
        { port: 3306, service: 'MySQL', banner: 'MySQL 5.7.38', info: '数据库服务，存储教学数据' },
      ],
    },
    connectResults: {
      'teach.cassel.edu:80': '连接成功！[HTTP/1.1 200 OK] nginx 1.18 — 教务系统 Web 服务正常运行。服务器返回了版本号和状态码 200，表示一切正常。',
    },
    curlResults: null,
    exploitResults: null,
    score: 70,
  ...
  'ch1-2': {
    ...
    index: 1,
    title: '侦察实战',
    story: '实战考核！教学楼 dorm.cassel.edu 和核心服务器 server.cassel.edu 都有异常流量，你需要全面扫描这两台服务器，并连接核心服务器的关键端口确认服务信息。全部侦察完成后输入 report 提交报告。',
    isBoss: true,
    objectives: [
      { id: 'scan_dorm', description: '扫描教学楼服务器 scan dorm.cassel.edu' },
      { id: 'scan_server', description: '扫描核心服务器 scan server.cassel.edu' },
    ...
    hints: [
      '侦察的第一步是什么？试试 scan dorm.cassel.edu',
      '还有一台服务器需要扫描。试试 scan server.cassel.edu',
      '端口扫描完了，用 connect <target> <port> 逐一连接核心服务器的端口。先试试 connect server.cassel.edu 22',
      '继续连接：connect server.cassel.edu 80',
      '最后一个端口：connect server.cassel.edu 3306',
    ...
    scanResults: {
      ...
      'server.cassel.edu': [
        ...
        { port: 80, service: 'HTTP', banner: 'Apache 2.4.46', info: '学院官网主站，版本较旧' },
        { port: 3306, service: 'MySQL', banner: 'MySQL 5.7.38', info: '核心数据库，不应暴露在公网！' },
      ],
    },
    connectResults: {
      'server.cassel.edu:22': '连接成功！[SSH-2.0-OpenSSH_7.4] 远程管理服务。建议使用密钥认证而非密码登录。',
      'server.cassel.edu:80': '连接成功！[HTTP/1.1 200 OK] Apache 2.4.46 — 学院官网。服务器版本较旧，可能存在已知漏洞。',
      'server.cassel.edu:3306': '连接成功！[MySQL 5.7.38] 数据库服务暴露在公网，风险极高！需要立即处理。',
    },
    curlResults: null,
    exploitResults: null,
    score: 100,
  ...
  'ch2-1': {
    ...
    index: 0,
    title: '网站的结构',
    story: '学院官网最近被人挂上了奇怪的页面，我们需要查看网站的结构和响应信息。用 curl 命令来探索 Web 服务。',
    isBoss: false,
    objectives: [
      { id: 'curl_page', description: '使用 curl cassel.edu 获取学院官网页面内容' },
      { id: 'curl_headers', description: '查看服务器响应头: curl cassel.edu --headers' },
      { id: 'curl_admin', description: '探索隐藏路径: curl cassel.edu/admin' },
      { id: 'exploit_brute', description: '用发现的凭据登录: exploit cassel.edu/admin brute-login' },
    ],
    target: null,
    expectedCommands: ['curl', 'exploit'],
    mentorMessages: [
      { trigger: 'start', text: 'Web安全是网络安全的重要战场。curl 命令就像浏览器的"技术眼"——能看到网页背后的源代码和服务器信息。试试 curl cassel.edu 看看学院官网的 HTML 源码。' },
      { trigger: 'cmd:curl', text: 'HTML 源码里藏着注释！开发者留下了管理员入口 /admin 和密码 admin123。再用 curl cassel.edu --headers 看看 HTTP 响应头，它会暴露服务器软件和版本号。' },
      { trigger: 'cmd:curl_headers', text: '看到 Server 和 X-Powered-By 头了吗？攻击者会用这些版本号搜索已知漏洞。再用 curl cassel.edu/admin 访问隐藏的管理后台——看看开发者还暴露了什么。' },
      { trigger: 'cmd:curl_admin', text: '登录页面连验证都没做！开发者的注释写着"暂未加验证"。现在用我们发现的凭据尝试登录：exploit cassel.edu/admin brute-login ——把刚才找到的 admin:admin123 用起来！' },
      { trigger: 'cmd:exploit', text: '登录成功！看到了吗？你在 curl 阶段搜集到的信息在 exploit 阶段变成了实际的攻击入口。这就是信息收集的价值——每条线索都可能通向新的攻击面。记住这个流程，下一关你会用同样的思路去发现和利用漏洞。' },
    ],
    knowledgeCards: [
      ...
      { trigger: 'cmd:exploit', text: '信息收集→漏洞利用的联动：curl 发现凭据→exploit 使用凭据登录。真实渗透测试中，每个侦察结果都可能成为下一步攻击的跳板。' },
    ],
    scanResults: null,
    connectResults: null,
    exploitResults: {
      'cassel.edu/admin:brute-login': '🎉 登录成功！\n\n使用凭据 admin / admin123 成功登录管理后台。\n\n管理后台功能:\n  1. 用户管理 — 查看/编辑所有用户信息\n  2. 内容发布 — 发布/修改学院新闻\n  3. 系统设置 — 修改网站配置\n\n💡 这就是信息收集的价值！curl 阶段找到的密码注释，在 exploit 阶段变成了管理后台的钥匙。真实攻击中，攻击者会不断循环这个"收集→利用"的过程，逐步深入系统。',
    },
    curlResults: {
      'cassel.edu': '<!DOCTYPE html>\n<html>\n<head>\n  <title>卡塞尔学院官网</title>\n</head>\n<body>\n  <!-- TODO: 移除调试信息 -->\n  <h1>欢迎来到 卡塞尔学院</h1>\n  <p>学院新闻：<a href="/news">查看最新动态</a></p>\n  <!-- 管理员入口: /admin 密码: admin123 -->\n  <footer>(c) 2025 卡塞尔学院</footer>\n</body>\n</html>',
      'cassel.edu:headers': 'HTTP/1.1 200 OK\nServer: Apache/2.4.46 (Unix)\nX-Powered-By: PHP/7.4.33\nSet-Cookie: PHPSESSID=abc123; path=/\nContent-Type: text/html; charset=UTF-8',
      'cassel.edu/admin': '<!DOCTYPE html>\n<html>\n<head>\n  <title>管理后台 - 卡塞尔学院</title>\n</head>\n<body>\n  <h1>管理员登录</h1>\n  <form action="/admin/login" method="POST">\n    <input type="text" name="username" placeholder="用户名">\n    <input type="password" name="password" placeholder="密码">\n    <button type="submit">登录</button>\n  </form>\n  <!-- 开发中，暂未加验证 -->\n</body>\n</html>',
    },
    score: 100,
    badge: null,
    completionCriteria: { type: 'all_objectives' },
  ...
  'ch2-2': {
    ...
    index: 1,
    title: 'Web攻防实战',
    story: '学院官网存在严重的 SQL 注入漏洞，搜索框没有对用户输入做过滤。你需要发现漏洞、利用它获取数据库信息，然后用 patch 命令修复漏洞。全部完成后输入 report 提交。',
    isBoss: true,
    objectives: [
      { id: 'curl_page', description: '查看学院官网: curl cassel.edu' },
      { id: 'sql_discover', description: '利用 SQL 注入漏洞: exploit cassel.edu/search sql-inject' },
      { id: 'patch_sanitize', description: '修复输入过滤漏洞: patch cassel.edu sanitize-input' },
    ...
    hints: [
      '先用 curl cassel.edu 看看学院官网页面，找找线索',
      '用 exploit cassel.edu/search sql-inject 测试搜索框有没有 SQL 注入漏洞',
      '漏洞确认了！用 patch cassel.edu sanitize-input 修复输入过滤',
  ...
  'ch5-1': {
    ...
    index: 0,
    title: '信息收集',
    story: '学院间网络安全对抗赛开始了！你的任务是攻破靶机 target.ctf.cassel.edu，先收集尽可能多的情报。',
    isBoss: false,
    objectives: [
      { id: 'scan_target', description: '扫描靶机开放端口 scan target.ctf.cassel.edu' },
      { id: 'curl_target', description: '查看靶机 Web 服务内容 curl target.ctf.cassel.edu' },
      { id: 'connect_80', description: '连接 Web 端口确认服务: connect target.ctf.cassel.edu 80' },
    ],
    target: null,
    expectedCommands: ['scan', 'curl', 'connect'],
    mentorMessages: [
      { trigger: 'start', text: 'CTF(Capture The Flag)是网络安全竞赛的经典形式。核心原则：永远先侦察再攻击！用 scan target.ctf.cassel.edu 扫描端口，curl 查看网页，connect 确认服务细节。信息越多，攻击路径越多。' },
      { trigger: 'cmd:scan', text: '两个端口：80是标准Web服务，2222是非标准SSH端口——管理员故意换了端口号想隐藏它。现在用 curl target.ctf.cassel.edu 抓取网页源码，看看有没有隐藏线索。' },
      { trigger: 'cmd:curl', text: '注意到HTML注释吗？默认密码admin:admin、隐藏参数role=user——都是开发者不小心留下的。再用 connect target.ctf.cassel.edu 80 直接跟Web服务握个手，看看它怎么回应。' },
    ...
    connectResults: {
      'target.ctf.cassel.edu:80': '连接成功！[HTTP/1.1 200 OK] nginx 1.22 — CTF 挑战靶机 Web 服务。登录页面正在运行，准备好迎接挑战。',
    },
    curlResults: {
      'target.ctf.cassel.edu': '<!DOCTYPE html>\n<html>\n<head>\n  <title>CTF Challenge - Login</title>\n</head>\n<body>\n  <h1>CTF 挑战靶机</h1>\n  <form action="/login" method="POST">\n    <input type="text" name="username" placeholder="用户名">\n    <input type="password" name="password" placeholder="密码">\n    <button type="submit">登录</button>\n  </form>\n  <!-- 提示: 试试默认密码 admin:admin -->\n  <!-- 隐藏参数: <input type="hidden" name="role" value="user"> -->\n</body>\n</html>',
    },
    exploitResults: null,
    score: 100,
    badge: null,
  ...
  'ch5-2': {
    ...
    index: 1,
    title: '终极挑战',
    story: '最后一关！目标: ultimate.ctf.cassel.edu。这台终极靶机集成了多层防护，你需要综合运用所有已学技能完成完整攻击链，找到最终 flag。每步操作如果连续3次没有进展，我会给你提示。',
    isBoss: true,
    objectives: [
    ...
    connectResults: {
      'ultimate.ctf.cassel.edu:22': '连接成功！[SSH-2.0-OpenSSH_8.9] 标准 SSH 服务，需要有效凭据。',
      'ultimate.ctf.cassel.edu:80': '连接成功！[HTTP/1.1 200 OK] Apache 2.4.51 — 静态企业官网页面。',
      'ultimate.ctf.cassel.edu:8080': '连接成功！[HTTP/1.1 200 OK] Apache Tomcat 9.0 — 管理后台登录页面。⚠️ 版本存在已知漏洞！默认凭据可能未修改。',
      'ultimate.ctf.cassel.edu:3306': '连接成功！[MySQL 8.0] 数据库服务，使用默认端口 3306。',
    },
    curlResults: {
